...
We're defining which remote repositories emacs will download packages from below. I've temporarily added the =gnu-devel= archive for macOS only as =magit= won't install otherwise.
...
Below are any settings where I'm not sure to put in this file just yet.
...
I'll need to take a deeper look at the font config in the future as I'd like to implement the code so that, at least in org files, source blocks always contain fixed-width fonts, but regular text is variable width (a-la MS word). I believe the System Crafters video on org config has that info in it.
...
For some reason, Emacs is still autosaving files in the same directory as file that is being edited. I thought I disabled that with the code below, but I'll need to look into this further as this causes some annoyances with git (unstanged files).
...
We'll define the default completion styles here before adding features:
...
This only sort of works because it doesn't work with symlinks. Need to find a better solution. I believe the System Crafters videos on dired might have something better to offer but I'll need to review them.
...
Additional modules are included with the package but need to be loaded explicitly for use. Below, we're enabling th =org-habit= module to allow habit tracking in the agenda view.
...
=org-capture= allows quick capture using templates into your existing files. So far, we have templates in place for contacts, tasks, next tasks, entries to check out (like links or articles), and a metrics capture that can currently quickly take my weight and add it to a table.
