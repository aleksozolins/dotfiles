...
;; Set org-agenda files to all files in the denote/agenda dir
;; It might be better in the future to create a simple function to set this value to all files in
;; denote-subdirectory that have the "_agenda" keyword instead
...
(setq org-capture-templates
      `(("c" "Contact" entry (file+headline "~/docs/denote/20220727T132509--contacts__contact.org" "Misc"),
        ...
        ("t" "Task")
        ("tt" "Task" entry (file+olp "~/docs/denote/agenda/20210804T113317--todos__agenda.org" "Inbox")
...
(setq org-default-notes-file (concat org-directory "~/docs/denote/agenda/inbox.txt"))
...
(defun my-denote-add-to-agenda ()
  ...
  (interactive)
  ;; 1. Check if the current file is in denote-directory
  (if (and (buffer-file-name)
           (string-prefix-p denote-directory (buffer-file-name))
           (string-suffix-p ".org" (buffer-file-name)))
      (progn
        ;; 2. Add #+category: project if not present
        (goto-char (point-min))
        ...
        (let* ((current-path (buffer-file-name))
               (new-path (concat denote-directory "agenda/" (file-name-nondirectory current-path))))
          (rename-file current-path new-path 1) ; 1 means OK-IF-ALREADY-EXISTS
          (set-visited-file-name new-path) ; update the buffer's associated file
          (set-buffer-modified-p nil))  ; mark the buffer as not modified
        ;; 4. Set org-agenda-files
        (setq org-agenda-files 
              (cl-remove-if
               (lambda (file) (string-match-p "/\\.#" file))
               (directory-files-recursively (concat denote-directory "agenda") "\\(\\.org\\'\\|\\.txt\\'\\)"))))
    (message "The current file is not in denote-directory or is not an org file.")))
...
(defun my-denote-remove-from-agenda ()
  ...
  (interactive)
  ;; 1. Check if the current file is in denote-directory/agenda
  (if (and (buffer-file-name)
           (string-prefix-p (concat denote-directory "agenda/") (buffer-file-name))
           (string-suffix-p ".org" (buffer-file-name)))
      (progn
        ;; 2. Remove any #+category: line
        (goto-char (point-min))
        ...
        ;; 3. Move the file
        (let* ((current-path (buffer-file-name))
               (new-path (concat denote-directory "/" (file-name-nondirectory current-path))))
          (rename-file current-path new-path 1) ; 1 means OK-IF-ALREADY-EXISTS
          (set-visited-file-name new-path) ; update the buffer's associated file
          (set-buffer-modified-p nil))  ; mark the buffer as not modified
